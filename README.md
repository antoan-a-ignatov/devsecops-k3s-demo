...
Early framing of this problem risked conflating "restrict which policies can be attached to a role" (achievable with `Condition` keys, already done for `AttachRolePolicy`/`PassRole`) with "restrict what a role can inline onto itself" (`PutRolePolicy`, which takes an arbitrary policy document as a parameter — no condition key can inspect its contents). A Permissions Boundary solves the second case differently: it doesn't gate the *write*, it gates the *effect*. Getting this distinction right up front shaped the whole design — including realizing the same `Condition`-context-key gap already hit once with `AttachRolePolicy`/`PassRole` would recur on `iam:CreateRole`'s `iam:PermissionsBoundary` condition if left in a combined statement, so it was split out preemptively rather than rediscovered through another failed pipeline run.
